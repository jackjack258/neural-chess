...
def create_model():
    ...
    model.compile(optimizer=tf.optimizers.legacy.Adam(learning_rate=.002), loss=tf.keras.losses.mean_absolute_error, metrics=['mae'])
    return model
...
train_dataset = tf.data.Dataset.load("train_datasetv39", compression='GZIP')
print(train_dataset.snapshot)
val_dataset = tf.data.Dataset.load("train_datasetv39", compression='GZIP')
...
model.fit(train_dataset, validation_data=val_dataset, epochs=2000, callbacks=[loss_callback,  checkpoint_callback])
model.save('test.h5')
